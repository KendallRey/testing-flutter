...
class ListService {
  CollectionReference getUserListItems(String userId) {
    return FirebaseFirestore.instance
        .collection(FirestoreCollections.list)
        .doc(userId)
        .collection(FirestoreCollections.listItems);
  }
  ...
  Stream<List<Map<String, dynamic>>> getUserItems(String userId) {
    final collection =
        getUserListItems(userId).orderBy(Model.createdAtKey, descending: true);
  ...
  Future<void> updateUserItem(
      String userId, String id, ListItemModel item) async {
    try {
      isLoading = true;
      final collection = getUserListItems(userId);
      final payload = await item.insertToMapEncryptedAsync();
